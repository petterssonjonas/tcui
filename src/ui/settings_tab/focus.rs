...
pub(super) fn cycle_edit_popup_focus(popup: &mut EditProvidersPopupState, len: usize, forward: bool) {
    if len == 0 {
        popup.focus = None;
        return;
    }
...
pub(super) fn move_edit_popup_vertically(popup: &mut EditProvidersPopupState, len: usize, forward: bool) {
    if len == 0 {
        popup.focus = None;
        return;
    }
